...
#include "kdtree.h"
#include "malloc.h"
#include "parameters_viscous_res.h"
...
DEFINE_PROFILE(vis_res, vol_thread, i)
{
 ...
 if (current_time == 0.0)   // replace first_iteration
 {
    ...
    double a;
    double dist;
    double min_dist = dist_threshold * 1.1;
    double slope = (core_vis_res - wall_vis_res) / dist_threshold;
    ...
    begin_c_loop_int(c, vol_thread) // Loop over all cells in each compute node
    {
        ...
        if (dist < dist_threshold)
            a = wall_vis_res + slope * dist;
        else
            a = core_vis_res;

        C_PROFILE(c, vol_thread, i) = a;
        C_UDMI(c, vol_thread, 0) = a;
