...
/* user must create a three UDM in fluent before compiling*/
...
#include "kdtree.h"
#include "malloc.h"
#include "bilinear_parameters_twoSurface.h"


DEFINE_PROFILE(porosity, vol_thread, i)
{
#if !RP_HOST
    real current_time;
 ...
 if (current_time == 0.0)   /* replace first_iteration */
 {
    ...
    int aneurysm_face_ID = aneurysm_surf_id;
    int neck_face_ID = neck_surf_id;
    Thread *aneurysm_boundary_thread = Lookup_Thread(domain, aneurysm_face_ID);
    Thread *neck_boundary_thread = Lookup_Thread(domain, neck_face_ID);
    
    int n_faces_neck = THREAD_N_ELEMENTS_INT(neck_boundary_thread);
    int n_faces_aneurysm = THREAD_N_ELEMENTS_INT(aneurysm_boundary_thread);


    int n_faces = n_faces_neck + n_faces_aneurysm;
    int n_faces2 = n_faces_neck + n_faces_aneurysm;
    ...
    /* recieve number of faces at node 0 to compute total number of faces */
        ...
        if(I_AM_NODE_ZERO_P)
        {
            ...
            Message0("Total Number of faces: %d" , total_num_faces);
        }
    #endif
    ...
    int noface = 0;
    int num_face_node = 0;
    begin_f_loop(f, aneurysm_boundary_thread)
    {
        F_CENTROID(x_f_array[f], f, aneurysm_boundary_thread);
        noface++;
        num_face_node++;
    }
    end_f_loop(f, aneurysm_boundary_thread)

    begin_f_loop(f, neck_boundary_thread)
    {
        F_CENTROID(x_f_array[f + num_face_node], f, neck_boundary_thread);
        noface++;
    }
    end_f_loop(f, neck_boundary_thread)
    ...
    /* Collect all face coordinate data at node zero */
        ...
        if(I_AM_NODE_ZERO_P)
        {
            ...
            begin_f_loop(f, aneurysm_boundary_thread)
            {            
                F_CENTROID(x_f_array_total[f], f, aneurysm_boundary_thread);
                num_face_node++;
            }
            end_f_loop(f, aneurysm_boundary_thread)
            begin_f_loop(f, neck_boundary_thread)
            {
                F_CENTROID(x_f_array_total[f+num_face_node], f, neck_boundary_thread);
    ...
    begin_c_loop_int(c, vol_thread) /* Loop over all cells in each compute node */
    {
        ...
        C_PROFILE(c, vol_thread, i) = phi;
        C_UDMI(c, vol_thread, 0) = phi;
